...
$varNames$ must contain a vector of parameter names that need to be analysed. By providing the parameter names, the user must make sure that no extra paramdeters (e.g., time and ID) are passed to the $flowMeans$ function as it will not remove them automatically.
